...
$migrationPath = Join-Path $projectRoot "database\migrations\0012_assistant_history.sql"
$verificationPath = Join-Path $projectRoot "database\queries\verify_0012_assistant_history.sql"
...
if ($missingObjects) { throw "0012 requires IAM migration 0011 first; missing: $missingObjects" }

Get-Content -LiteralPath $migrationPath -Raw -Encoding UTF8 |
    & $dockerPath @composeArgs exec -T postgres sh -c `
        'psql -v ON_ERROR_STOP=1 -U "$POSTGRES_USER" -d "$POSTGRES_DB"'
if ($LASTEXITCODE -ne 0) { throw "Migration 0012_assistant_history failed." }

Get-Content -LiteralPath $verificationPath -Raw -Encoding UTF8 |
    & $dockerPath @composeArgs exec -T postgres sh -c `
        'psql -v ON_ERROR_STOP=1 -U "$POSTGRES_USER" -d "$POSTGRES_DB"'
if ($LASTEXITCODE -ne 0) { throw "Migration 0012 applied but verification failed." }

Write-Output "Migration 0012_assistant_history applied and verified."
